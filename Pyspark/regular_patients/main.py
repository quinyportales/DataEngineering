...
if __name__ == '__main__':
    ...
    FILE_PATH = "file:///Pyspark/regular_patients/input/enroll.csv"
    #creating the spark session and loading the data
    patients = PatientsData(FILE_PATH)

    #instantiating my logger
    logger = Log4J(patients.spark)
    logger.info('Starting PatientsData')
    ...
    OUTPUT_PATH = "file:///Pyspark/regular_patients/output"
    df_patients.write.mode("overwrite").csv(OUTPUT_PATH)
    df_patients.show(10)
    df_patients.printSchema()

    #stop spark
    logger.info('Finished PatientsData')
    patients.spark.stop()
